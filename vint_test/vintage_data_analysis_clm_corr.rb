...
CSV.open(res_csv_name, "w") do |csv|
  ...
  sort_vint.each_with_index do |vint_rec, index|
    ...
    hdd = vint_rec["geography"]["hdd"] #HDD
    csv_out << hdd
    ...
    pop_frac_info = clim_pop.select {|pop_info| (hdd >= pop_info["min_hdd18"]) && (hdd <= pop_info["max_hdd18"])}
    unless pop_frac_info.empty?
      csv_out << pop_frac_info[0]["pop_frac"]
    end
