...
cost_files = Dir['/home/osdev/costing_output/**/qaqc.json']
cost_files.each do |cost_file|
  ...
  out_json_file = './results/' + building_name + '_' + template[0]["value"].to_s + '_' + fuel + '_' + province + '.json'
  loc = /run/ =~ cost_file
  in_osm_file = cost_file[0..(loc + 3)] + 'in.osm'
  out_osm_file = './results/' + building_name + '_' + template[0]["value"].to_s + '_' + fuel + '_' + province + '.osm'
